...
func (r *Resource) GetCurrentState(ctx context.Context, obj interface{}) (interface{}, error) {
	...
	if isKVM(obj) {
		serviceNames = append(serviceNames, serviceNameWorker)
	}
	...
	if isKVM(obj) {
		kvmConfig, err := toKVMConfig(obj)
		if err != nil {
			return nil, microerror.Mask(err)
		}

		if isDeleted(kvmConfig) {
			n := clusterNamespace(clusterID)
